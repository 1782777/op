...
def impliedVolatility(className, args, callPrice=None, putPrice=None, high=500.0, low=0.0):
	...
	decimals = 4
	for i in range(50):	# To avoid infinite loops 10000/ 0.00001
		mid = (high + low) / 2
		if mid < 0.00001:
			mid = 0.00001
		if callPrice:
...
class GK:
	...
	def __init__(self, args, volatility=None, callPrice=None, putPrice=None, \
		...
		self.daysToExpiration = float(args[4]) / YEAR_DAY
	...
	def _theta(self):
		...
		put = -self.underlyingPrice * _b_ * norm.pdf(self._d1_) * \
				...
				norm.cdf(-self._d1_) + self.domesticRate * self.strikePrice * \
				_b_ * norm.cdf(-self._d2_)
		return [call / YEAR_DAY, put / YEAR_DAY]
...
class BS:
	...
	def __init__(self, args, volatility=None, callPrice=None, putPrice=None, \
		...
		self.daysToExpiration = float(args[3]) / YEAR_DAY
...
if __name__ =='__main__':
	bs = BS([3.051,3.4,3,65],volatility=18.066, callPrice=0.01)
	print (bs._delta,bs.gamma,bs.vega,bs._theta)